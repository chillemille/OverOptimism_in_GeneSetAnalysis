...
# function arguments
# - term: gene set (i.e., its ID) of which the rank shall be extracted
# - GSEA_results: GSEA results table in which the gene'sets rank is to be determined
# - ranking: either "ranking" or "padj":
#           "p_adj": get gene set's adjusted p-value (optimisation goal 2)
#           "ranking": get gene set's rank among all other gene sets (optimisation goal 3)
...
#required inputs:
# - geneset: gene set from KEGG or GO gene set database
# geneset_database: gene set database containing the gene set
# - expression_data: expression data set (in this case in the Ensembl gene ID format)
#-> gene ID conversion to Entrez is performed within the function
# - phenotype_labels: phenotype labels to assign the individual samples to the opposing conditions
#- metric: indicate whether a genes adjusted p-value ("p_value") or its rank among the
#remaining gene sets ("rank") shall be minimized
