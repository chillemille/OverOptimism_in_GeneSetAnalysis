...
padj_GeneSet1_truephen_long$gene_set <- ifelse(padj_GeneSet1_truephen_long$GSA_tool == "PADOG", 
                                                      "PrimImmun", 
                                                      "tCell") 
...
padj_GeneSet2_truephen_long$gene_set <- ifelse(padj_GeneSet1_truephen_long$GSA_tool == "PADOG", 
                                                      "GraftvsHost", 
                                                      "Demethylation")
...
padj_allgenesets_truephen_long$GSA_tool <- factor(padj_allgenesets_truephen_long$GSA_tool, 
...
padj_allgenesets_truephen_long$gene_set <- factor(padj_allgenesets_truephen_long$gene_set, 
...
padj_allgenesets_truephen_long$sig_threshold <- ifelse(grepl(paste(c("GSEAPreranked","\\bGSEA\\b"), collapse='|'),padj_allgenesets_truephen_long$GSA_tool), 
                                                              yes = 0.25, no = 0.05)
...
plot_adj_pickrell_truephens <- 
  ggplot(data =padj_allgenesets_truephen_long, 
         aes(x = interaction(GSA_tool, state, lex.order = TRUE), 
             y = padj, group = 1)) + 
  geom_line( aes(group=unique_ID, color = gene_set), size=0.5) + 
  geom_point(aes(color = gene_set),size = 1.5) + 
